      ...
      {isModalOpen && editingService && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0A0A0A] border border-white/10 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleSave} className="flex-1 overflow-y-auto custom-scrollbar flex flex-col relative">
              <div className="p-6 space-y-5 flex-1">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Título</label>
                    <input type="text" required value={editingService.title} onChange={e => {
                      const val = e.target.value;
                      const autoSlug = val.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9\s-]/g, '').replace(/[\s-]+/g, '-');
                      setEditingService({ ...editingService, title: val, slug: autoSlug });
              ...
              <div className="p-6 border-t border-white/10 shrink-0 bg-[#0A0A0A] flex justify-end gap-3 sticky bottom-0">
