...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch slides from Supabase; fall back to defaults if empty
    supabase
      .from('hero_slides')
      .select('*')
      .order('sort_order')
    ...
    <section id="hero" className="relative h-screen w-full overflow-hidden bg-black">
      ...
      <AnimatePresence mode='wait'>
        <motion.div
          ...
          className="absolute inset-0 z-0 gsap-hero-bg"
        >
      ...
      <div className="absolute bottom-10 left-0 right-0 z-30 flex justify-center gap-3">
        {slides.map((_, index) => (
          <button
            ...
            className={`h-1 rounded-full transition-all duration-300 ${index === currentSlide ? 'w-12 bg-raynold-red shadow-[0_0_10px_#E60000]' : 'w-4 bg-white/30 hover:bg-white/50'
              }`}
          />
        ))}
      </div>
