...
import { Plus, Edit2, Trash2, ShieldAlert, Save, X, Search, Phone, Mail, MapPin } from 'lucide-react';
...
const AdminSuppliers = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('raynold_suppliers');
    if (saved) {
  ...
  const saveSuppliers = (newSuppliers: Supplier[]) => {
    ...
    localStorage.setItem('raynold_suppliers', JSON.stringify(newSuppliers));
  };
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (exists) {
      saveSuppliers(suppliers.map(s => s.id === editingSupplier.id ? editingSupplier : s));
    } else {
      saveSuppliers([...suppliers, editingSupplier]);
    }
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
            ...
            <button 
              ...
              className="px-6 py-3 btn-animated font-bold rounded-lg flex items-center justify-center gap-2 whitespace-nowrap"
            >
              <Plus size={18} />
              Nuevo Proveedor
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSuppliers.map((supplier) => (
            <div key={supplier.id} className="bg-[#0A0A0A] border border-white/10 rounded-xl p-6 hover:border-white/20 transition-colors relative group">
              ...
              {supplier.taxId && (
                <p className="text-xs text-raynold-green font-mono mb-4">RNC/NIT: {supplier.taxId}</p>
              )}
              ...
              <div className="space-y-3 mt-4">
                {supplier.contactName && (
                  <div className="flex items-center gap-3 text-sm text-gray-300">
                    <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center shrink-0">
                      <span className="font-bold text-white">{supplier.contactName.charAt(0)}</span>
                    </div>
                ...
                {supplier.phone && (
                  <div className="flex items-center gap-3 text-sm text-gray-400">
                    ...
                    <a href={`tel:${supplier.phone}`} className="hover:text-white transition-colors">{supplier.phone}</a>
                  </div>
                )}
                {supplier.email && (
                  <div className="flex items-center gap-3 text-sm text-gray-400">
                    <Mail size={16} className="text-gray-500" />
                    <a href={`mailto:${supplier.email}`} className="hover:text-white transition-colors truncate">{supplier.email}</a>
                  </div>
                )}
                {supplier.address && (
          ...
          <div className="relative w-full max-w-2xl bg-[#0A0A0A] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
              <h2 className="text-xl font-bold text-white font-futuristic">
                {suppliers.find(s => s.id === editingSupplier.id) ? 'Editar Proveedor' : 'Nuevo Proveedor'}
              </h2>
              <button 
